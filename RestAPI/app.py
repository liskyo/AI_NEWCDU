...
op_logger.info(f"測試路徑:{log_path}/restAPI")
data = {
    "sensor_value": {
...
messages = {
    ...
    "error": {
        ...
        "M344": ["PC1 Error", False],
        "M345": ["PLC Communication Broken Error", False],
    },
}
sensor_value_data = {
...
def read_data_from_json():
    global thrshd, ctr_data, measure_data, valve_setting, valve_factory
    ...
    with open(f"{json_path}/valve_setting.json", "r") as file:
...
def change_to_imperial():
    ...
    for key in key_list:
        if key == "oil_temp_set":
            op_logger.info(f'ctr_data["value"]["oil_temp_set"]: {ctr_data["value"]["oil_temp_set"]}')
        ...
        if key == "oil_pressure_set":
            op_logger.info(f'ctr_data["value"]["oil_pressure_set"]: {ctr_data["value"]["oil_pressure_set"]}')
...
class CduOpMode(Resource):
    ...
    def patch(self):
        ...
        op_logger.info(f"op_mode updated successfully {op_mode}")
        return {
    ...
    def get(self):
        ...
        try:
            with open(f"{json_path}/sensor_data.json", "r") as json_file:
                ...
                data["sensor_value"]["temp_coolant_supply"] = round(
                    data1["value"]["temp_clntSply"], 2
                )
                data["sensor_value"]["temp_coolant_supply_spare"] = round(
                    data1["value"]["temp_clntSplySpr"], 2
                )
                data["sensor_value"]["temp_coolant_return"] = round(
                    data1["value"]["temp_clntRtn"], 2
                )
                data["sensor_value"]["temp_ambient"] = round(
                    data1["value"]["temp_ambient"], 2
                )
                data["sensor_value"]["relative_humid"] = round(
                    data1["value"]["rltHmd"], 2
                )
                data["sensor_value"]["dew_point"] = round(data1["value"]["dewPt"], 2)
                data["sensor_value"]["temp_water_supply"] = round(
                    data1["value"]["temp_waterIn"], 2
                )
                data["sensor_value"]["temp_water_return"] = round(
                    data1["value"]["temp_waterOut"], 2
                )
                data["sensor_value"]["pressure_coolant_supply"] = round(
                    data1["value"]["prsr_clntSply"], 2
                )
                data["sensor_value"]["pressure_coolant_supply_spare"] = round(
                    data1["value"]["prsr_clntSplySpr"], 2
                )
                data["sensor_value"]["pressure_coolant_return"] = round(
                    data1["value"]["prsr_clntRtn"], 2
                )
                data["sensor_value"]["pressure_filter_in"] = round(
                    data1["value"]["prsr_fltIn"], 2
                )
                data["sensor_value"]["pressure_filter1_out"] = round(
                    data1["value"]["prsr_flt1Out"], 2
                )
                data["sensor_value"]["pressure_filter2_out"] = round(
                    data1["value"]["prsr_flt2Out"], 2
                )
                data["sensor_value"]["pressure_filter3_out"] = round(
                    data1["value"]["prsr_flt3Out"], 2
                )
                data["sensor_value"]["pressure_filter4_out"] = round(
                    data1["value"]["prsr_flt4Out"], 2
                )
                data["sensor_value"]["pressure_filter5_out"] = round(
                    data1["value"]["prsr_flt5Out"], 2
                )
                data["sensor_value"]["pressure_water_in"] = round(
                    data1["value"]["prsr_wtrIn"], 2
                )
                data["sensor_value"]["pressure_water_out"] = round(
                    data1["value"]["prsr_wtrOut"], 2
                )
                data["sensor_value"]["flow_coolant"] = round(
                    data1["value"]["flow_clnt"], 2
                )
                data["sensor_value"]["flow_water"] = round(
                    data1["value"]["flow_wtr"], 2
                )
                data["sensor_value"]["pH"] = round(data1["value"]["pH"], 2)
                data["sensor_value"]["conductivity"] = round(data1["value"]["cdct"], 2)
                data["sensor_value"]["Turbidity"] = round(data1["value"]["tbd"], 2)
                data["sensor_value"]["heat_capacity"] = round(
                    data1["value"]["heat_capacity"], 2
                )
                data["sensor_value"]["power_comsume"] = round(
                    data1["value"]["power"], 2
                )
                data["sensor_value"]["average_current"] = round(data1["value"]["AC"], 2)
...
class PumpSpeed(Resource):
    ...
    def patch(self):
        ...
        if ctr_data["value"]["resultMode"] in ["Stop"]:
            return "In Stop mode can not set pump speed ."
   
        if 0 < pump1_speed < 25  :
            return{"message": "Pump1 Speed invalid input. The accepted setting range is 25 - 100 or 0."
            }, 400           
        if 0 < pump2_speed < 25  :
            return{"message": "Pump1 Speed invalid input. The accepted setting range is 25 - 100 or 0."
            }, 400     
        ...
        if pump1_speed > 0 and pump2_speed > 0 and pump1_speed != pump2_speed:
            return {
                "message": "Please synchronize speeds to run both pumps or change speed."
            }, 400
        else:
            try:
                with ModbusTcpClient(host=modbus_host, port=modbus_port) as client:
            ...
            op_logger.info(
                f"Pump speed updated successfully. pump speed {pump_speed_set}"
            )
            return {
...
class WaterPV(Resource):
    ...
    def patch(self):
        ...
        op_logger.info(
            f"Water proportional valve position updated successfully. {water_pv_set}"
        )
        return {
    ...
    def get(self):
        ...
        try:
            with open(f"{json_path}/ctr_data.json", "r") as json_file:
                ...
                temp_set["temp_set"] = round(data["value"]["oil_temp_set"], 2)
        except Exception as e:
...
class PressureSet(Resource):
    ...
    def get(self):
        ...
        try:
            with open(f"{json_path}/ctr_data.json", "r") as json_file:
                data = json.load(json_file)
                pressure_set["pressure_set"] = round(
                    data["value"]["oil_pressure_set"], 2
                )
        except Exception as e:
            print(f"get mode error: {e}")
            return jsonify({"message": "PLC Communication Error"}), 500
    ...
    def patch(self):
        """Update the pressure setting in 0 kpa - 750 kpa (0 psi to 108.75 psi)"""
    ...
    def get(self):
        ...
        try:
            with open(f"{json_path}/ctr_data.json", "r") as json_file:
                ...
                data["pump_service_hours"]["pump1_service_hours"] = data1["text"][
                    "Pump1_run_time"
                ]

                data["pump_service_hours"]["pump2_service_hours"] = data1["text"][
                    "Pump2_run_time"
                ]
...
class filter_Service_hours(Resource):
    ...
    def get(self):
        ...
        with open(f"{json_path}/ctr_data.json", "r") as json_file:
            data1 = json.load(json_file)
            data["filter_service_hours"]["filter_service_hours"] = data1["filter"]["f1"]
            data["filter_service_hours"]["filter2_service_hours"] = data1["filter"][
                "f2"
            ]
            data["filter_service_hours"]["filter3_service_hours"] = data1["filter"][
                "f3"
            ]
            data["filter_service_hours"]["filter4_service_hours"] = data1["filter"][
                "f4"
            ]
            data["filter_service_hours"]["filter5_service_hours"] = data1["filter"][
                "f5"
            ]
