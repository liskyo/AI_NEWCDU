...
def trap(trap_bool_lists, v2):
    ...
    base_offsets = [0, 16, 32, 48, 64, 80, 96, 112, 128]
    global index, pc2_active

    try:
        with open(f"{os.path.dirname(log_path)}/PLC/json/pc_status.json", "r") as file:
            pc2_active = json.load(file)
    except Exception as e:
        print(f"read pc2 active status error: {e}")
            ...
            try:
                with ModbusTcpClient(
                    ...
                    if not trap_list.isError():
                        ...
                        error_section = [
                            trap_bool_lists[i] for i in [0, 1, 2, 5, 6, 7, 8, 9, 10]
                        ]
